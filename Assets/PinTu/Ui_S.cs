...
public class Ui_S : MonoBehaviour {
    ...
    void Start () {
        ...
        print("The correct order of image names" + all_SpritName);
        ...
        {//根据图片集，实例化按钮
            ...
            instan_Btn.transform.localPosition = new Vector3(instan_Btn.transform.localPosition.x, instan_Btn.transform.localPosition.y,0f);
            instan_Btn.transform.localEulerAngles = Vector3.zero;
        }
        ...
        StartCoroutine(PingtuTip()); ;
    }
    IEnumerator PingtuTip() 
    {
        yield return new WaitForSeconds(0.5f);
        EventManager.Instance.TipChange?.Invoke("Please complete the puzzle！！！");
    ...
    public void Btn_OnClick(RectTransform btn_Rect)
    {
        ...
        {
            print("Similar to the empty image, replace the image of clicking the button on the image with the Sprit image of the empty image");
    ...
    void BtnName_All()
    {
        ...
        {
            EventManager.Instance.TipChange?.Invoke("Please search for password clues in the room！");
